 ...
 * - Always include ALL headers from the CSV in every JSON object.
 *   If a row is missing a value for a header, that key is included with an empty string "".
 * - Default pretty-printing is ON (indent = 2). You can still override with --pretty <n>.
 * - Columns are treated as plain text by default (no type inference unless --infer-types).
 *
 * Usage (single file):
...
Optional (both modes):
  -o, --output <path>         File (file mode) or output root directory (dir mode).
      ...
      --pretty [n]            Pretty-print JSON with indentation (default 2 if flag omitted; override with a number)
...
Notes:
  • Every JSON row will include ALL headers from the CSV. Missing values become "" (empty string).
  • By default, all columns are treated as plain text (no inference) and empty strings are preserved.
`;
...
// Default pretty-printing ON (2 spaces). If --pretty <n> is provided, use that.
let pretty = 2;
if (hasFlag('pretty')) {
  const p = getOpt('pretty');
  if (p && !p.startsWith('-')) {
    const num = Number(p);
    if (!Number.isNaN(num)) pretty = num;
  }
}

// Keep plain text by default; only infer when explicitly requested.
...
function coerceValue(key, value) {
  ...
  const unwrap = (s) => {
    ...
    // Trim whitespace first to avoid ' "text" ' patterns
  ...
  // Always preserve empty strings; treat as plain text
  if (trimmed === '') return '';

  // Array splitting for selected fields (only if explicitly opted in)
  if (arrayFields.size > 0 && arrayFields.has(key.toLowerCase())) {
    const parts = trimmed.split(arraySep).map(s => s.trim());
    return parts;
  }

  if (!inferTypes) return trimmed;

  // Type inference (only when --infer-types is passed)
  if (/^-?\\d+$/.test(trimmed)) {
...
async function convertFile(fileInputPath, fileOutputPath) {
  ...
  function writeRow(obj) {
    ...
    // Ensure ALL headers exist; missing values => ""
    const fullObj = {};
    for (const h of headers) {
      const v = Object.prototype.hasOwnProperty.call(obj, h) ? obj[h] : '';
      fullObj[h] = (v === undefined || v === null) ? '' : v;
    }

    if (ndjson) {
      outStream.write(JSON.stringify(fullObj));
    ...
    const json = JSON.stringify(fullObj, null, pretty || 0);
    if (pretty > 0) {
